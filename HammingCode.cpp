...
public:
	...
	string msg;	  // it will store the all bits (data + redundant). We made it dynamic because at compile time we dont know how much redundant bits will be there, we will initialize memory to it once we know the number of redundant bits.
	hamming(string data)
	{
		this->data = data;
	...
	void setRedundantBits()
	{
		...
		for (int i = 1; i <= m + r; i *= 2)
		{
			...
			for (int j = i + 1; j <= m + r; j++)
			{
				...
				{
					if (msg[j] == '1')
						count++; // counting the number of ones in corresponding data bits
				}
			}
...
int main()
{
	...
	cout << "Checking for error in 111010010111" << endl;
	h.receiver();
	cout << "Checking for error in 111010011111" << endl;
	h.msg[9] = '1';
	h.receiver();
